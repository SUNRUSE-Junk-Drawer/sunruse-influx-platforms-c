describe "platforms", ->
	describe "javascript", ->
		describe "codeCache", ->
			...
			describe "imports", ->
				it "toolchain", ->
					expect(codeCache.toolchain).toBe require "sunruse-influx-toolchain"
			describe "on calling", ->
				result = toolchain = existingA = existingB = existingC = cache = undefined
				beforeEach ->
					existingA = 
						code: "Test Code A"
						value: "Existing A"
					...
					toolchain = codeCache.toolchain
					codeCache.toolchain = 
						valuesEquivalent: jasmine.createSpy()
					codeCache.toolchain.valuesEquivalent.and.callFake (platform, a, b) ->
